...
class ClickHouseDialect(default.DefaultDialect):
    ...
    def __init__(self, **kwargs):
        super(ClickHouseDialect, self).__init__(**kwargs)
#        self.paramstyle = 'pyformat'
        self.driver = 'chdb'
        supports_statement_cache = True  # Add this line to enable statement caching
        paramstyle = 'qmark'  # Change the paramstyle to 'qmark'

    @classmethod
    def import_dbapi(cls: Type["Dialect"]):
        connector = dbapi
        return connector
    ...
    def _get_default_schema_name(self, connection):
        conn = dbapi.connect().cursor()
        return conn.execute('select currentDatabase()')
